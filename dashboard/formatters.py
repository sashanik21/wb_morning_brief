...
CONFIRMATION_LABELS = {
    "carttoorderpercent": "Конверсия в заказ снизилась",
    "realsellablestock": "Нет доступного остатка",
    "ordercount": "Снизилось количество заказов",
    "ordersum": "Снизилась выручка",
    "ctr": "Снизился CTR рекламы",
    "addtocartpercent": "Снизилась конверсия в корзину",
    "cpc": "Выросла стоимость клика",
    "drr": "Вырос ДРР",
    "wbstocks": "Товар отсутствует на складе",
}
CONFIRMATION_TOOLTIPS = {
    "carttoorderpercent": (
        "Показатель рассчитан по данным Funnel.\n\n"
        "Система обнаружила снижение доли покупателей, которые оформляют заказ "
        "после добавления товара в корзину."
    ),
    "realsellablestock": (
        "По данным остатков товар недоступен для продажи.\n"
        "Причина определена по данным stocks_daily."
    ),
    "ordercount": (
        "Количество заказов уменьшилось относительно базы сравнения.\n\n"
        "Источник:\ndaily_funnel."
    ),
    "ordersum": (
        "Выручка уменьшилась относительно базы сравнения.\n\n"
        "Источник:\ndaily_funnel."
    ),
    "ctr": (
        "Реклама получает меньше кликов на показы.\n\n"
        "Источник:\ndaily_ads_metrics."
    ),
    "addtocartpercent": (
        "Показатель рассчитан по данным Funnel.\n\n"
        "Система обнаружила снижение доли покупателей, которые добавляют товар в корзину."
    ),
    "cpc": (
        "Стоимость клика выросла относительно базы сравнения.\n\n"
        "Источник:\ndaily_ads_metrics."
    ),
    "drr": (
        "Доля рекламных расходов выросла относительно базы сравнения.\n\n"
        "Источник:\ndaily_ads_metrics."
    ),
    "wbstocks": (
        "Товар отсутствует на складе Wildberries и недоступен покупателям.\n\n"
        "Источник:\nstocks_daily."
    ),
}
CONFIRMATION_COLUMN_HELP = (
    "Главное подтверждение показывает показатель, который сильнее всего повлиял "
    "на определение причины проблемы."
)
MAIN_REASON_HELP = (
    "Главная причина определяется по суммарным потерям среди проблемных SKU "
    "за выбранный период."
)
REASON_DESCRIPTION_HELP = (
    "Описание формируется автоматически на основе найденных проблем и доступных данных."
)
...
REASON_EXPLANATIONS = {
    ...
    "реклама": "Проблема в рекламном трафике или его качестве. Проверяются кликабельность рекламы, стоимость клика, доля рекламных расходов, рекламные заказы и покрытие рекламы.",
    "реклама остановлена": "Рекламная кампания остановлена или не даёт трафик. Нужно проверить статус кампании, бюджет и ставки.",
    "остатки": "Проблема с доступностью товара. Проверяются доступный остаток, остатки на складах, риск отсутствия товара и поставки.",
...
REASON_TABLE_HINTS = {
    ...
    "реклама": "проверить кликабельность рекламы, стоимость клика, долю рекламных расходов, ставки и кампании",
...
def _confirmation_key(value):
    return str(value or "").strip().replace("_", "").lower()


def confirmation_label(value):
    key = _confirmation_key(value)
    return CONFIRMATION_LABELS.get(key, "Требует управленческой проверки" if key else "Не определено")


def confirmation_tooltip(value):
    key = _confirmation_key(value)
    return CONFIRMATION_TOOLTIPS.get(
        key,
        "Система нашла подтверждение причины, но для него пока нет отдельного описания.",
    )
        ...
        records.append(
            {
                ...
                "главное подтверждение": confirmation_label(
                    first_present(row, ["impact_confidence", "report_trust_score", "reportTrustScore"], "")
                ),
                "подсказка подтверждения": confirmation_tooltip(
                    first_present(row, ["impact_confidence", "report_trust_score", "reportTrustScore"], "")
                ),
